...
(function (global, factory) {
  ...
  function accumulateDeepEqualDiffs(a, b, diffsAccumulator) {
    ...
    if (lodash.isSet(a) && lodash.isSet(b)) {
      ...
      var setItemDiffs = [];
      var numberOfDeepEqualsValues = 0;

      var aValues = _toConsumableArray(a.values());

      var bValues = _toConsumableArray(b.values());

      for (var _i = setLength; _i--; _i > 0) {
        var _diffEquals = accumulateDeepEqualDiffs(aValues[_i], bValues[_i], setItemDiffs, "".concat(pathString, ".values()[").concat(_i, "]"), {
          detailed: detailed
        });
      ...
      if (detailed || numberOfDeepEqualsValues !== setLength) {
        diffsAccumulator.push.apply(diffsAccumulator, setItemDiffs);
      }
    ...
    if (lodash.isMap(a) && lodash.isMap(b)) {
      ...
      var mapKeysDiffs = [];
      var numberOfDeepEqualsKeys = 0;

      var aKeys = _toConsumableArray(a.keys());

      var bKeys = _toConsumableArray(b.keys());

      for (var _i2 = mapLength; _i2--; _i2 > 0) {
        var _diffEquals2 = accumulateDeepEqualDiffs(aKeys[_i2], bKeys[_i2], mapKeysDiffs, "".concat(pathString, ".keys()[").concat(_i2, "]"), {
          detailed: detailed
        });
      ...
      if (detailed || numberOfDeepEqualsKeys !== mapLength) {
        diffsAccumulator.push.apply(diffsAccumulator, mapKeysDiffs);
      }
      ...
      var mapValuesDiffs = [];
      var _numberOfDeepEqualsValues = 0;

      var _aValues = _toConsumableArray(a.values());

      var _bValues = _toConsumableArray(b.values());

      for (var _i3 = mapLength; _i3--; _i3 > 0) {
        var _diffEquals3 = accumulateDeepEqualDiffs(_aValues[_i3], _bValues[_i3], mapValuesDiffs, "".concat(pathString, ".values()[").concat(_i3, "]"), {
          detailed: detailed
        });

        if (_diffEquals3) {
          _numberOfDeepEqualsValues++;
        }
      }

      if (detailed || _numberOfDeepEqualsValues !== mapLength) {
        diffsAccumulator.push.apply(diffsAccumulator, mapValuesDiffs);
      }

      if (_numberOfDeepEqualsValues === mapLength) {
    ...
    if (_typeof(a) === 'object' && _typeof(b) === 'object' && Object.getPrototypeOf(a) === Object.getPrototypeOf(b)) {
      var _aKeys = Object.getOwnPropertyNames(a);

      var _bKeys = Object.getOwnPropertyNames(b);

      var allKeys = lodash.uniq([].concat(_toConsumableArray(_aKeys), _toConsumableArray(_bKeys)));
      var clonedA = lodash.isPlainObject(a) ? _objectSpread2({}, a) : a;
      var clonedB = lodash.isPlainObject(b) ? _objectSpread2({}, b) : b;

      if (allKeys.length !== _aKeys.length || allKeys.length !== _bKeys.length) {
      ...
      for (var _i4 = keysLength; _i4--; _i4 > 0) {
        if (!lodash.has(b, relevantKeys[_i4])) {
          return trackDiff(clonedA, clonedB, diffsAccumulator, pathString, diffTypes.different);
        }
      }
      ...
      for (var _i5 = keysLength; _i5--; _i5 > 0) {
        var key = relevantKeys[_i5];
